...
Client Side routing is much quicker than server side routing. client side is basically just mapping different pages to different urls but each page is in the browsers memory. where as server side routing requires a request to fetch data from a server which takes longer.
